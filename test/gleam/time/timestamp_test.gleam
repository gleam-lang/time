import gleam/int
import gleam/list
import gleam/order
import gleam/result
import gleam/string
import gleam/time/calendar.{August, Date, December, January, TimeOfDay}
import gleam/time/duration
import gleam/time/generators
import gleam/time/timestamp
import qcheck
import simplifile

pub fn compare_property_0_test() {
  use #(x, y) <- qcheck.given(qcheck.tuple2(
    qcheck.uniform_int(),
    qcheck.uniform_int(),
  ))
  let tx = timestamp.from_unix_seconds(x)
  let ty = timestamp.from_unix_seconds(y)
  assert timestamp.compare(tx, ty) == int.compare(x, y)
}

pub fn compare_0_test() {
  assert timestamp.compare(
      timestamp.from_unix_seconds(1),
      timestamp.from_unix_seconds(1),
    )
    == order.Eq
}

pub fn compare_1_test() {
  assert timestamp.compare(
      timestamp.from_unix_seconds(2),
      timestamp.from_unix_seconds(1),
    )
    == order.Gt
}

pub fn compare_2_test() {
  assert timestamp.compare(
      timestamp.from_unix_seconds(2),
      timestamp.from_unix_seconds(3),
    )
    == order.Lt
}

pub fn difference_0_test() {
  assert timestamp.difference(
      timestamp.from_unix_seconds(1),
      timestamp.from_unix_seconds(1),
    )
    == duration.seconds(0)
}

pub fn difference_1_test() {
  assert timestamp.difference(
      timestamp.from_unix_seconds(1),
      timestamp.from_unix_seconds(5),
    )
    == duration.seconds(4)
}

pub fn difference_2_test() {
  assert timestamp.difference(
      timestamp.from_unix_seconds_and_nanoseconds(1, 10),
      timestamp.from_unix_seconds_and_nanoseconds(5, 20),
    )
    == duration.add(duration.seconds(4), duration.nanoseconds(10))
}

pub fn add_property_0_test() {
  use #(x, y) <- qcheck.given(qcheck.tuple2(
    qcheck.uniform_int(),
    qcheck.uniform_int(),
  ))
  let expected = timestamp.from_unix_seconds_and_nanoseconds(0, x + y)
  let actual =
    timestamp.from_unix_seconds_and_nanoseconds(0, x)
    |> timestamp.add(duration.nanoseconds(y))
  assert expected == actual
}

pub fn add_property_1_test() {
  use #(x, y) <- qcheck.given(qcheck.tuple2(
    qcheck.uniform_int(),
    qcheck.uniform_int(),
  ))
  let expected = timestamp.from_unix_seconds_and_nanoseconds(x + y, 0)
  let actual =
    timestamp.from_unix_seconds_and_nanoseconds(x, 0)
    |> timestamp.add(duration.seconds(y))
  assert expected == actual
}

pub fn add_0_test() {
  assert timestamp.add(timestamp.from_unix_seconds(0), duration.seconds(1))
    == timestamp.from_unix_seconds(1)
}

pub fn add_1_test() {
  assert timestamp.add(timestamp.from_unix_seconds(100), duration.seconds(-1))
    == timestamp.from_unix_seconds(99)
}

pub fn add_2_test() {
  assert timestamp.add(
      timestamp.from_unix_seconds(99),
      duration.nanoseconds(100),
    )
    == timestamp.from_unix_seconds_and_nanoseconds(99, 100)
}

pub fn add_3_test() {
  assert timestamp.add(
      timestamp.from_unix_seconds_and_nanoseconds(0, -1),
      duration.nanoseconds(-1_000_000_000),
    )
    == timestamp.from_unix_seconds_and_nanoseconds(0, -1_000_000_001)
}

pub fn add_4_test() {
  assert timestamp.add(
      timestamp.from_unix_seconds_and_nanoseconds(0, 1),
      duration.nanoseconds(-1_000_000_000),
    )
    == timestamp.from_unix_seconds_and_nanoseconds(0, -999_999_999)
}

pub fn subtract_property_0_test() {
  use #(x, y) <- qcheck.given(qcheck.tuple2(
    qcheck.uniform_int(),
    qcheck.uniform_int(),
  ))
  let expected = timestamp.from_unix_seconds_and_nanoseconds(0, x - y)
  let actual =
    timestamp.from_unix_seconds_and_nanoseconds(0, x)
    |> timestamp.subtract(duration.nanoseconds(y))
  assert expected == actual
}

pub fn subtract_property_1_test() {
  use #(x, y) <- qcheck.given(qcheck.tuple2(
    qcheck.uniform_int(),
    qcheck.uniform_int(),
  ))
  let expected = timestamp.from_unix_seconds_and_nanoseconds(x - y, 0)
  let actual =
    timestamp.from_unix_seconds_and_nanoseconds(x, 0)
    |> timestamp.subtract(duration.seconds(y))
  assert expected == actual
}

pub fn subtract_0_test() {
  assert timestamp.subtract(timestamp.from_unix_seconds(0), duration.seconds(1))
    == timestamp.from_unix_seconds(-1)
}

pub fn subtract_1_test() {
  assert timestamp.subtract(
      timestamp.from_unix_seconds(100),
      duration.seconds(-1),
    )
    == timestamp.from_unix_seconds(101)
}

pub fn subtract_2_test() {
  assert timestamp.subtract(
      timestamp.from_unix_seconds(99),
      duration.nanoseconds(100),
    )
    == timestamp.from_unix_seconds_and_nanoseconds(98, 999_999_900)
}

pub fn subtract_3_test() {
  assert timestamp.subtract(
      timestamp.from_unix_seconds_and_nanoseconds(0, -1),
      duration.nanoseconds(-1_000_000_000),
    )
    == timestamp.from_unix_seconds_and_nanoseconds(0, 999_999_999)
}

pub fn subtract_4_test() {
  assert timestamp.subtract(
      timestamp.from_unix_seconds_and_nanoseconds(0, 1),
      duration.nanoseconds(-1_000_000_000),
    )
    == timestamp.from_unix_seconds_and_nanoseconds(1, 1)
}

pub fn to_unix_seconds_0_test() {
  assert timestamp.to_unix_seconds(timestamp.from_unix_seconds_and_nanoseconds(
      1,
      0,
    ))
    == 1.0
}

pub fn to_unix_seconds_1_test() {
  assert timestamp.to_unix_seconds(timestamp.from_unix_seconds_and_nanoseconds(
      1,
      500_000_000,
    ))
    == 1.5
}

pub fn to_unix_seconds_and_nanoseconds_0_test() {
  assert timestamp.to_unix_seconds_and_nanoseconds(
      timestamp.from_unix_seconds_and_nanoseconds(1, 0),
    )
    == #(1, 0)
}

pub fn to_unix_seconds_and_nanoseconds_1_test() {
  assert timestamp.to_unix_seconds_and_nanoseconds(
      timestamp.from_unix_seconds_and_nanoseconds(1, 2),
    )
    == #(1, 2)
}

pub fn system_time_0_test() {
  let #(now, _) =
    timestamp.system_time() |> timestamp.to_unix_seconds_and_nanoseconds

  // This test will start to fail once enough time has passed.
  // When that happens please update these values.
  let when_this_test_was_last_updated = 1_768_565_586
  let christmas_day_2027 = 1_829_692_800

  let assert True = now > when_this_test_was_last_updated
  let assert True = now < christmas_day_2027
}

pub fn to_rfc3339_0_test() {
  assert timestamp.to_rfc3339(
      timestamp.from_unix_seconds(1_735_309_467),
      calendar.utc_offset,
    )
    == "2024-12-27T14:24:27Z"
}

pub fn to_rfc3339_1_test() {
  assert timestamp.to_rfc3339(
      timestamp.from_unix_seconds(1),
      calendar.utc_offset,
    )
    == "1970-01-01T00:00:01Z"
}

pub fn to_rfc3339_2_test() {
  assert timestamp.to_rfc3339(
      timestamp.from_unix_seconds(0),
      calendar.utc_offset,
    )
    == "1970-01-01T00:00:00Z"
}

pub fn to_rfc3339_3_test() {
  assert timestamp.to_rfc3339(
      timestamp.from_unix_seconds(123_456_789),
      calendar.utc_offset,
    )
    == "1973-11-29T21:33:09Z"
}

pub fn to_rfc3339_4_test() {
  assert timestamp.to_rfc3339(
      timestamp.from_unix_seconds(31_560_000),
      calendar.utc_offset,
    )
    == "1971-01-01T06:40:00Z"
}

pub fn to_rfc3339_5_test() {
  assert timestamp.to_rfc3339(
      timestamp.from_unix_seconds(-12_345_678),
      calendar.utc_offset,
    )
    == "1969-08-11T02:38:42Z"
}

pub fn to_rfc3339_6_test() {
  assert timestamp.to_rfc3339(
      timestamp.from_unix_seconds(-1),
      calendar.utc_offset,
    )
    == "1969-12-31T23:59:59Z"
}

pub fn to_rfc3339_7_test() {
  assert timestamp.to_rfc3339(
      timestamp.from_unix_seconds(-60 * 60 - 60 * 5),
      duration.seconds(65 * 60),
    )
    == "1970-01-01T00:00:00+01:05"
}

pub fn to_rfc3339_8_test() {
  assert timestamp.to_rfc3339(
      timestamp.from_unix_seconds(0),
      duration.seconds(120 * 60),
    )
    == "1970-01-01T02:00:00+02:00"
}

pub fn to_rfc3339_9_test() {
  assert timestamp.to_rfc3339(
      timestamp.from_unix_seconds(-62_167_219_200),
      calendar.utc_offset,
    )
    == "0000-01-01T00:00:00Z"
}

pub fn to_rfc3339_10_test() {
  assert timestamp.to_rfc3339(
      timestamp.from_unix_seconds(-62_135_596_800),
      calendar.utc_offset,
    )
    == "0001-01-01T00:00:00Z"
}

pub fn to_rfc3339_11_test() {
  assert timestamp.to_rfc3339(
      timestamp.from_unix_seconds(-61_851_600_000),
      calendar.utc_offset,
    )
    == "0010-01-01T00:00:00Z"
}

pub fn to_rfc3339_12_test() {
  assert timestamp.to_rfc3339(
      timestamp.from_unix_seconds(-59_011_459_200),
      calendar.utc_offset,
    )
    == "0100-01-01T00:00:00Z"
}

pub fn to_rfc3339_13_test() {
  assert timestamp.to_rfc3339(
      timestamp.from_unix_seconds_and_nanoseconds(0, 1),
      calendar.utc_offset,
    )
    == "1970-01-01T00:00:00.000000001Z"
}

pub fn to_rfc3339_14_test() {
  assert timestamp.to_rfc3339(
      timestamp.from_unix_seconds_and_nanoseconds(-1, 12),
      calendar.utc_offset,
    )
    == "1969-12-31T23:59:59.000000012Z"
}

pub fn to_rfc3339_15_test() {
  assert timestamp.to_rfc3339(
      timestamp.from_unix_seconds_and_nanoseconds(1, 123),
      calendar.utc_offset,
    )
    == "1970-01-01T00:00:01.000000123Z"
}

pub fn to_rfc3339_16_test() {
  assert timestamp.to_rfc3339(
      timestamp.from_unix_seconds_and_nanoseconds(0, 1230),
      calendar.utc_offset,
    )
    == "1970-01-01T00:00:00.00000123Z"
}

pub fn to_rfc3339_17_test() {
  assert timestamp.to_rfc3339(
      timestamp.from_unix_seconds_and_nanoseconds(0, 500_600_000),
      calendar.utc_offset,
    )
    == "1970-01-01T00:00:00.5006Z"
}

pub fn to_rfc3339_18_test() {
  assert timestamp.to_rfc3339(
      timestamp.from_unix_seconds_and_nanoseconds(0, 500_006),
      calendar.utc_offset,
    )
    == "1970-01-01T00:00:00.000500006Z"
}

pub fn to_rfc3339_19_test() {
  assert timestamp.to_rfc3339(
      timestamp.from_unix_seconds_and_nanoseconds(0, 999_999_999),
      calendar.utc_offset,
    )
    == "1970-01-01T00:00:00.999999999Z"
}

pub fn to_rfc3339_20_test() {
  assert timestamp.to_rfc3339(
      timestamp.from_unix_seconds_and_nanoseconds(0, 0),
      calendar.utc_offset,
    )
    == "1970-01-01T00:00:00Z"
}

pub fn to_rfc3339_21_test() {
  assert timestamp.to_rfc3339(
      timestamp.from_unix_seconds_and_nanoseconds(0, 1_000_000_001),
      calendar.utc_offset,
    )
    == "1970-01-01T00:00:01.000000001Z"
}

// RFC 3339 Parsing

pub fn parse_rfc3339_0_test() {
  let assert Ok(timestamp) = timestamp.parse_rfc3339("1970-01-01T00:00:00.6Z")

  assert timestamp.to_unix_seconds_and_nanoseconds(timestamp)
    == #(0, 600_000_000)
}

pub fn parse_rfc3339_1_test() {
  let assert Ok(timestamp) = timestamp.parse_rfc3339("1969-12-31T23:59:59.6Z")

  assert timestamp.to_unix_seconds_and_nanoseconds(timestamp)
    == #(-1, 600_000_000)
}

pub fn parse_rfc3339_2_test() {
  let assert Ok(timestamp) =
    timestamp.parse_rfc3339("1970-01-01t00:00:00.55+00:01")

  assert timestamp.to_unix_seconds_and_nanoseconds(timestamp)
    == #(-60, 550_000_000)
}

pub fn parse_rfc3339_3_test() {
  let assert Ok(timestamp) =
    timestamp.parse_rfc3339("1970-01-01T00:00:00.55-00:01")

  assert timestamp.to_unix_seconds_and_nanoseconds(timestamp)
    == #(60, 550_000_000)
}

pub fn parse_rfc3339_4_test() {
  let assert Ok(timestamp) =
    timestamp.parse_rfc3339("1970-01-01 00:00:00-00:01")

  assert timestamp.to_unix_seconds_and_nanoseconds(timestamp) == #(60, 0)
}

pub fn timestamp_rfc3339_string_timestamp_roundtrip_property_test() {
  use timestamp <- qcheck.given(generators.timestamp())

  let assert Ok(parsed_timestamp) =
    timestamp
    |> timestamp.to_rfc3339(calendar.utc_offset)
    |> timestamp.parse_rfc3339

  assert timestamp == parsed_timestamp
}

pub fn rfc3339_string_timestamp_rfc3339_string_roundtrip_property_test() {
  use date_time <- qcheck.given(generators.rfc3339(
    with_leap_second: True,
    second_fraction_spec: generators.Default,
    avoid_erlang_errors: False,
  ))

  let assert Ok(original_timestamp) = timestamp.parse_rfc3339(date_time)

  let assert Ok(roundtrip_timestamp) =
    original_timestamp
    |> timestamp.to_rfc3339(calendar.utc_offset)
    |> timestamp.parse_rfc3339

  assert original_timestamp == roundtrip_timestamp
}

// Eastern US Timezone round trip tests
pub fn rfc3339_string_timestamp_rfc3339_string_roundtrip_to_est_test() {
  let assert Ok(date_time) =
    timestamp.parse_rfc3339("2025-02-04T13:00:00+00:00")

  assert timestamp.to_rfc3339(date_time, duration.seconds(-18_000))
    == "2025-02-04T08:00:00-05:00"
}

pub fn rfc3339_string_timestamp_rfc3339_string_roundtrip_from_est_test() {
  let assert Ok(date_time) =
    timestamp.parse_rfc3339("2025-02-04T13:00:00-05:00")

  assert timestamp.to_rfc3339(date_time, calendar.utc_offset)
    == "2025-02-04T18:00:00Z"
}

// Check against OCaml Ptime reference implementation.
//
// These test cases include leap seconds.
pub fn parse_rfc3339_examples_from_file_test() {
  let assert Ok(data) = simplifile.read("test/gleam/time/timestamps_parsed.tsv")
  data
  |> string.split(on: "\n")
  |> list.drop(1)
  |> list.each(fn(line) {
    case string.split(line, on: "\t") {
      [ts, seconds, nanos, _, _] -> {
        let assert Ok(expected_seconds) = int.parse(seconds)
        let assert Ok(expected_nanos) = int.parse(nanos)

        let assert Ok(parsed_ts) = timestamp.parse_rfc3339(ts)
        let #(parsed_seconds, parsed_nanos) =
          timestamp.to_unix_seconds_and_nanoseconds(parsed_ts)

        assert expected_seconds == parsed_seconds
        assert expected_nanos == parsed_nanos
      }
      // No more to parse.
      [""] -> Nil
      _ -> panic as "bad input line"
    }
  })
}

@external(erlang, "gleam_time_test_ffi", "rfc3339_to_system_time_in_milliseconds")
@external(javascript, "../../gleam_time_test_ffi.mjs", "rfc3339_to_system_time_in_milliseconds")
fn rfc3339_to_system_time_in_milliseconds(input: String) -> Result(Int, Nil)

// WARNING: This can give different values on Erlang and JS targets if you pass
// in a timestamp that has more than 3 fractional digits.  Erlang will give you
// nanosecond precision, but will round to nearest nanosecond.  JavaScript will
// give you millisecond precision and trucate to the nearest millisecond.  So,
// the caller must ensure good values are passed in.
fn parse_rfc3339_oracle(input: String) -> Result(timestamp.Timestamp, Nil) {
  use total_milliseconds <- result.map(rfc3339_to_system_time_in_milliseconds(
    input,
  ))
  // Break out the millisecond fraction first so that the conversion to
  // nanoseconds doesn't overflow for JS in the normalise function.
  let millisecond_fraction = total_milliseconds % 1000
  let whole_seconds = { total_milliseconds - millisecond_fraction } / 1000
  timestamp.from_unix_seconds_and_nanoseconds(
    seconds: whole_seconds,
    nanoseconds: millisecond_fraction * 1_000_000,
  )
}

pub fn parse_rfc3339_matches_oracle_example_0_test() {
  let date_time = "9999-12-31T23:59:59.999Z"

  assert timestamp.parse_rfc3339(date_time) == parse_rfc3339_oracle(date_time)
}

pub fn parse_rfc3339_matches_oracle_example_1_test() {
  let date_time = "1970-01-01T00:00:00.111Z"

  assert timestamp.parse_rfc3339(date_time) == parse_rfc3339_oracle(date_time)
}

pub fn parse_rfc3339_matches_oracle_example_2_test() {
  let date_time = "1970-01-01T00:00:00.000Z"

  assert timestamp.parse_rfc3339(date_time) == parse_rfc3339_oracle(date_time)
}

pub fn parse_rfc3339_matches_oracle_example_3_test() {
  let date_time = "1969-12-31T23:59:59.444Z"

  assert timestamp.parse_rfc3339(date_time) == parse_rfc3339_oracle(date_time)
}

pub fn parse_rfc3339_matches_oracle_example_4_test() {
  let date_time = "1969-12-31T23:59:58.666Z"

  assert timestamp.parse_rfc3339(date_time) == parse_rfc3339_oracle(date_time)
}

pub fn parse_rfc3339_matches_oracle_example_5_test() {
  let date_time = "0000-01-01T00:00:00Z"

  assert timestamp.parse_rfc3339(date_time) == parse_rfc3339_oracle(date_time)
}

// The oracle gives badarg on Erlang as it is beyond the range of the
// 0000-01-01T00:00:00Z limit.
@target(javascript)
pub fn parse_rfc3339_matches_oracle_example_6_test() {
  let date_time = "0000-01-01T00:00:00+00:01"

  assert timestamp.parse_rfc3339(date_time) == parse_rfc3339_oracle(date_time)
}

pub fn parse_rfc3339_matches_oracle_example_7_test() {
  let date_time = "0000-01-01T00:00:00-00:01"

  assert timestamp.parse_rfc3339(date_time) == parse_rfc3339_oracle(date_time)
}

pub fn parse_rfc3339_matches_oracle_example_8_test() {
  let date_time = "9999-12-31T23:59:59.999+00:01"

  assert timestamp.parse_rfc3339(date_time) == parse_rfc3339_oracle(date_time)
}

// This oracle gives badarg on Erlang as it is beyond the range of the
// 9999-12-31T23:59:59.999Z limit.
@target(javascript)
pub fn parse_rfc3339_matches_oracle_example_9_test() {
  let date_time = "9999-12-31T23:59:59.999-00:01"

  assert timestamp.parse_rfc3339(date_time) == parse_rfc3339_oracle(date_time)
}

// JS returns NaN for any leap seconds, so skip this test in JS.
@target(erlang)
pub fn parse_rfc3339_matches_oracle_example_10_test() {
  let date_time = "1970-01-01T23:59:60Z"

  assert timestamp.parse_rfc3339(date_time) == parse_rfc3339_oracle(date_time)
}

pub fn parse_rfc3339_matches_oracle_property_test() {
  use date_time <- qcheck.given(generators.rfc3339(
    // JavaScript oracle cannot handle leap-seconds.
    with_leap_second: False,
    // JavaScript oracle has max precision of milliseconds.
    second_fraction_spec: generators.WithMaxLength(3),
    // Some valid timestamps cannot be parsed by the Erlang oracle.
    avoid_erlang_errors: True,
  ))

  assert timestamp.parse_rfc3339(date_time) == parse_rfc3339_oracle(date_time)
}

pub fn parse_rfc3339_succeeds_for_valid_inputs_property_test() {
  use date_time <- qcheck.given(generators.rfc3339(
    with_leap_second: True,
    second_fraction_spec: generators.Default,
    avoid_erlang_errors: False,
  ))

  let assert Ok(value) = timestamp.parse_rfc3339(date_time)
  let _ = value
  Nil
}

pub fn parse_rfc3339_fails_for_invalid_inputs_test() {
  // The chance of randomly generating a valid RFC 3339 string is quite low....
  use string <- qcheck.given(qcheck.string())

  assert timestamp.parse_rfc3339(string) == Error(Nil)
}

pub fn parse_rfc3339_bad_leapyear_test() {
  // 29 days in February in a non-leap year is an error.
  assert timestamp.parse_rfc3339("2023-02-29T00:00:00Z") == Error(Nil)
}

pub fn parse_rfc3339_truncates_too_many_fractional_seconds_0_test() {
  let assert Ok(ts) = timestamp.parse_rfc3339("1970-01-01T00:00:00.1234567899Z")

  let #(_, nanoseconds) = timestamp.to_unix_seconds_and_nanoseconds(ts)

  assert nanoseconds == 123_456_789
}

pub fn parse_rfc3339_truncates_too_many_fractional_seconds_1_test() {
  let assert Ok(ts) = timestamp.parse_rfc3339("1970-01-01T00:00:00.1234567891Z")

  let #(_, nanoseconds) = timestamp.to_unix_seconds_and_nanoseconds(ts)

  assert nanoseconds == 123_456_789
}

// Examples from the docs

pub fn parse_rfc3339_docs_example_0_test() {
  let assert Ok(ts) =
    timestamp.parse_rfc3339("1970-01-01T00:00:01.12345678999Z")
  assert timestamp.to_unix_seconds_and_nanoseconds(ts) == #(1, 123_456_789)
}

pub fn parse_rfc3339_docs_example_1_test() {
  let assert Ok(ts) = timestamp.parse_rfc3339("2025-01-10t15:54:30-05:15")
  assert timestamp.to_unix_seconds_and_nanoseconds(ts) == #(1_736_543_370, 0)
}

// Bad timestamps fail to parse

pub fn parse_rfc3339_returns_error_for_bad_timestamp_0_test() {
  let assert Error(_) = timestamp.parse_rfc3339("12349-01-01T00:00:00Z")
}

pub fn parse_rfc3339_returns_error_for_bad_timestamp_1_test() {
  let assert Error(_) = timestamp.parse_rfc3339("1234-019-01T00:00:00Z")
}

pub fn parse_rfc3339_returns_error_for_bad_timestamp_2_test() {
  let assert Error(_) = timestamp.parse_rfc3339("1234-01-019T00:00:00Z")
}

pub fn parse_rfc3339_returns_error_for_bad_timestamp_3_test() {
  let assert Error(_) = timestamp.parse_rfc3339("1234-01-01 00:00:00ZZ")
}

pub fn parse_rfc3339_returns_error_for_bad_timestamp_4_test() {
  let assert Error(_) = timestamp.parse_rfc3339("1234-01-01T009:00:00Z")
}

pub fn parse_rfc3339_returns_error_for_bad_timestamp_5_test() {
  let assert Error(_) = timestamp.parse_rfc3339("1234-01-01T00:009:00Z")
}

pub fn parse_rfc3339_returns_error_for_bad_timestamp_6_test() {
  let assert Error(_) = timestamp.parse_rfc3339("1234-01-01T00:00:009Z")
}

pub fn parse_rfc3339_returns_error_for_bad_timestamp_7_test() {
  let assert Error(_) = timestamp.parse_rfc3339("1234-01-01T00:00:00*00:00")
}

pub fn parse_rfc3339_returns_error_for_bad_timestamp_8_test() {
  let assert Error(_) = timestamp.parse_rfc3339("1234-01-01T00:00:00+009:00")
}

pub fn parse_rfc3339_returns_error_for_bad_timestamp_9_test() {
  let assert Error(_) = timestamp.parse_rfc3339("1234-01-01T00:00:00+00:009")
}

pub fn parse_rfc3339_docs_example_2_test() {
  let assert Error(Nil) = timestamp.parse_rfc3339("1995-10-31")
}

pub fn normalise_negative_millis_test() {
  assert timestamp.from_unix_seconds_and_nanoseconds(1, -1_000_000_000)
    == timestamp.from_unix_seconds_and_nanoseconds(0, 0)
}

pub fn normalise_negative_millis_1_test() {
  assert timestamp.from_unix_seconds_and_nanoseconds(1, -1_400_000_000)
    == timestamp.from_unix_seconds_and_nanoseconds(-1, 600_000_000)
}

pub fn normalise_negative_millis_2_test() {
  assert timestamp.from_unix_seconds_and_nanoseconds(1, -2_600_000_000)
    == timestamp.from_unix_seconds_and_nanoseconds(-2, 400_000_000)
}

pub fn normalise_negative_millis_3_test() {
  assert timestamp.from_unix_seconds_and_nanoseconds(0, -1_000_000_000)
    == timestamp.from_unix_seconds_and_nanoseconds(-1, 0)
}

pub fn normalise_negative_millis_4_test() {
  assert timestamp.from_unix_seconds_and_nanoseconds(0, -1_400_000_000)
    == timestamp.from_unix_seconds_and_nanoseconds(-2, 600_000_000)
}

pub fn normalise_negative_millis_5_test() {
  assert timestamp.from_unix_seconds_and_nanoseconds(0, -2_600_000_000)
    == timestamp.from_unix_seconds_and_nanoseconds(-3, 400_000_000)
}

pub fn normalise_negative_millis_6_test() {
  assert timestamp.from_unix_seconds_and_nanoseconds(-1, -1_000_000_000)
    == timestamp.from_unix_seconds_and_nanoseconds(-2, 0)
}

pub fn normalise_negative_millis_7_test() {
  assert timestamp.from_unix_seconds_and_nanoseconds(-1, -1_400_000_000)
    == timestamp.from_unix_seconds_and_nanoseconds(-3, 600_000_000)
}

pub fn normalise_negative_millis_8_test() {
  assert timestamp.from_unix_seconds_and_nanoseconds(-1, -2_600_000_000)
    == timestamp.from_unix_seconds_and_nanoseconds(-4, 400_000_000)
}

pub fn normalise_positive_millis_test() {
  assert timestamp.from_unix_seconds_and_nanoseconds(1, 1_000_000_000)
    == timestamp.from_unix_seconds_and_nanoseconds(2, 0)
}

pub fn normalise_positive_millis_1_test() {
  assert timestamp.from_unix_seconds_and_nanoseconds(1, 1_400_000_000)
    == timestamp.from_unix_seconds_and_nanoseconds(2, 400_000_000)
}

pub fn normalise_positive_millis_2_test() {
  assert timestamp.from_unix_seconds_and_nanoseconds(1, 2_600_000_000)
    == timestamp.from_unix_seconds_and_nanoseconds(3, 600_000_000)
}

pub fn normalise_positive_millis_3_test() {
  assert timestamp.from_unix_seconds_and_nanoseconds(0, 1_000_000_000)
    == timestamp.from_unix_seconds_and_nanoseconds(1, 0)
}

pub fn normalise_positive_millis_4_test() {
  assert timestamp.from_unix_seconds_and_nanoseconds(0, 1_400_000_000)
    == timestamp.from_unix_seconds_and_nanoseconds(1, 400_000_000)
}

pub fn normalise_positive_millis_5_test() {
  assert timestamp.from_unix_seconds_and_nanoseconds(0, 2_600_000_000)
    == timestamp.from_unix_seconds_and_nanoseconds(2, 600_000_000)
}

pub fn normalise_positive_millis_6_test() {
  assert timestamp.from_unix_seconds_and_nanoseconds(-1, 1_000_000_000)
    == timestamp.from_unix_seconds_and_nanoseconds(0, 0)
}

pub fn normalise_positive_millis_7_test() {
  assert timestamp.from_unix_seconds_and_nanoseconds(-1, 1_400_000_000)
    == timestamp.from_unix_seconds_and_nanoseconds(0, 400_000_000)
}

pub fn normalise_positive_millis_8_test() {
  assert timestamp.from_unix_seconds_and_nanoseconds(-1, 2_600_000_000)
    == timestamp.from_unix_seconds_and_nanoseconds(1, 600_000_000)
}

pub fn from_calendar_0_test() {
  assert timestamp.to_rfc3339(
      timestamp.from_calendar(
        date: Date(year: 2024, month: December, day: 25),
        time: TimeOfDay(hours: 12, minutes: 30, seconds: 50, nanoseconds: 0),
        offset: duration.empty,
      ),
      duration.empty,
    )
    == "2024-12-25T12:30:50Z"
}

pub fn from_calendar_1_test() {
  assert timestamp.to_rfc3339(
      timestamp.from_calendar(
        date: Date(year: 50, month: August, day: 2),
        time: TimeOfDay(hours: 4, minutes: 10, seconds: 2, nanoseconds: 0),
        offset: duration.seconds(120),
      ),
      duration.empty,
    )
    == "0050-08-02T04:08:02Z"
}

pub fn from_calendar_2_test() {
  assert timestamp.to_rfc3339(
      timestamp.from_calendar(
        date: Date(year: 2024, month: December, day: 25),
        time: TimeOfDay(
          hours: 12,
          minutes: 30,
          seconds: 50,
          nanoseconds: 1_000_000,
        ),
        offset: duration.empty,
      ),
      duration.empty,
    )
    == "2024-12-25T12:30:50.001Z"
}

pub fn to_calendar_0_test() {
  assert timestamp.to_calendar(
      timestamp.from_unix_seconds(0),
      calendar.utc_offset,
    )
    == #(
      Date(year: 1970, month: January, day: 1),
      TimeOfDay(hours: 0, minutes: 0, seconds: 0, nanoseconds: 0),
    )
}

pub fn calendar_roundtrip_test() {
  use timestamp1 <- qcheck.given(generators.timestamp())
  let #(date, time_of_day) =
    timestamp.to_calendar(timestamp1, calendar.utc_offset)
  let timestamp2 =
    timestamp.from_calendar(date, time_of_day, calendar.utc_offset)
  assert timestamp1 == timestamp2
}

pub fn unix_epoch_test() {
  assert timestamp.from_unix_seconds(0) == timestamp.unix_epoch
}

pub fn to_http_date_0_test() {
  assert timestamp.to_http_date(timestamp.from_unix_seconds(0))
    == "Thu, 01 Jan 1970 00:00:00 GMT"
}

pub fn to_http_date_1_test() {
  let december_27_2024 = timestamp.from_unix_seconds(1_735_309_467)

  assert timestamp.to_http_date(december_27_2024)
    == "Fri, 27 Dec 2024 14:24:27 GMT"
}

pub fn to_http_date_2_test() {
  let two_days_after_epoch =
    timestamp.add(timestamp.unix_epoch, duration.hours(24 * 2))

  assert timestamp.to_http_date(two_days_after_epoch)
    == "Sat, 03 Jan 1970 00:00:00 GMT"
}

pub fn to_http_date_3_test() {
  let three_days_after_epoch =
    timestamp.add(timestamp.unix_epoch, duration.hours(24 * 3))

  assert timestamp.to_http_date(three_days_after_epoch)
    == "Sun, 04 Jan 1970 00:00:00 GMT"
}

pub fn to_http_date_4_test() {
  let four_days_after_epoch =
    timestamp.add(timestamp.unix_epoch, duration.hours(24 * 4))

  assert timestamp.to_http_date(four_days_after_epoch)
    == "Mon, 05 Jan 1970 00:00:00 GMT"
}

pub fn to_http_date_5_test() {
  let five_days_after_epoch =
    timestamp.add(timestamp.unix_epoch, duration.hours(24 * 5))

  assert timestamp.to_http_date(five_days_after_epoch)
    == "Tue, 06 Jan 1970 00:00:00 GMT"
}

pub fn to_http_date_6_test() {
  let six_days_after_epoch =
    timestamp.add(timestamp.unix_epoch, duration.hours(24 * 6))

  assert timestamp.to_http_date(six_days_after_epoch)
    == "Wed, 07 Jan 1970 00:00:00 GMT"
}

pub fn to_http_date_discards_nanoseconds_test() {
  let almost_one_second_after_epoch =
    timestamp.from_unix_seconds_and_nanoseconds(0, 999_999_999)

  assert timestamp.to_http_date(almost_one_second_after_epoch)
    == "Thu, 01 Jan 1970 00:00:00 GMT"
}

pub fn parse_http_date_imf_fixdate_test() {
  let december_27_2024 = timestamp.from_unix_seconds(1_735_309_467)

  assert timestamp.parse_http_date("Fri, 27 Dec 2024 14:24:27 GMT")
    == Ok(december_27_2024)
}

pub fn parse_http_date_rfc850_test() {
  let december_27_2024 = timestamp.from_unix_seconds(1_735_309_467)

  assert timestamp.parse_http_date("Friday, 27-Dec-24 14:24:27 GMT")
    == Ok(december_27_2024)
}

pub fn parse_http_date_asctime_test() {
  let december_27_2024 = timestamp.from_unix_seconds(1_735_309_467)

  assert timestamp.parse_http_date("Fri Dec 27 14:24:27 2024")
    == Ok(december_27_2024)
}

pub fn parse_http_date_asctime_space_padded_day_test() {
  let expected =
    timestamp.from_calendar(
      Date(2024, calendar.December, 7),
      TimeOfDay(14, 24, 27, 0),
      calendar.utc_offset,
    )

  assert timestamp.parse_http_date("Sat Dec  7 14:24:27 2024") == Ok(expected)
}

pub fn parse_http_date_ignores_mismatched_weekday_test() {
  let expected =
    timestamp.from_calendar(
      Date(2026, calendar.July, 17),
      TimeOfDay(1, 23, 45, 0),
      calendar.utc_offset,
    )
  assert timestamp.parse_http_date("Mon, 17 Jul 2026 01:23:45 GMT")
    == Ok(expected)
}

pub fn parse_http_date_accepts_leap_second_test() {
  let assert Ok(value) =
    timestamp.parse_http_date("Sat, 31 Dec 2016 23:59:60 GMT")

  let expected =
    timestamp.from_calendar(
      Date(2017, January, 1),
      TimeOfDay(0, 0, 0, 0),
      calendar.utc_offset,
    )
  assert value == expected
}

pub fn expand_rfc850_year_window_test() {
  assert timestamp.expand_rfc850_year_relative_to(year: 25, current_year: 2000)
    == 2025
  assert timestamp.expand_rfc850_year_relative_to(year: 50, current_year: 2000)
    == 2050
  assert timestamp.expand_rfc850_year_relative_to(year: 51, current_year: 2000)
    == 1951
  assert timestamp.expand_rfc850_year_relative_to(year: 0, current_year: 2099)
    == 2100
}

pub fn parse_http_date_returns_error_for_bad_timestamp_0_test() {
  assert timestamp.parse_http_date("RandomString, 17 Jul 2026 01:23:45 GMT")
    == Error(Nil)
}

pub fn parse_http_date_returns_error_for_bad_timestamp_1_test() {
  assert timestamp.parse_http_date("Funday, 17-Jul-26 01:23:45 GMT")
    == Error(Nil)
}

pub fn parse_http_date_returns_error_for_bad_timestamp_2_test() {
  assert timestamp.parse_http_date("Bad Jul 17 01:23:45 2026") == Error(Nil)
}

pub fn parse_http_date_returns_error_for_bad_timestamp_3_test() {
  assert timestamp.parse_http_date("Wed, 30 Feb 2026 00:00:00 GMT")
    == Error(Nil)
}

pub fn parse_http_date_returns_error_for_bad_timestamp_4_test() {
  assert timestamp.parse_http_date("Mon, 29 Jun 2026 24:00:00 GMT")
    == Error(Nil)
}

pub fn parse_http_date_returns_error_for_bad_timestamp_5_test() {
  assert timestamp.parse_http_date("Mon, 29 Jun 2026 23:60:00 GMT")
    == Error(Nil)
}

pub fn parse_http_date_returns_error_for_bad_timestamp_6_test() {
  assert timestamp.parse_http_date("Mon, 29 Jun 2026 23:59:61 GMT")
    == Error(Nil)
}

pub fn parse_http_date_returns_error_for_bad_timestamp_7_test() {
  assert timestamp.parse_http_date("Mon, 29 Jun 2026 01:49:08 UTC")
    == Error(Nil)
}

pub fn parse_http_date_returns_error_for_bad_timestamp_8_test() {
  assert timestamp.parse_http_date("Mon, 29 Jun 2026 01:49:08 GMT trailing")
    == Error(Nil)
}

pub fn parse_http_date_returns_error_for_bad_timestamp_9_test() {
  assert timestamp.parse_http_date("Mon, 9 Jun 2026 01:49:08 GMT") == Error(Nil)
}

pub fn parse_http_date_returns_error_for_bad_timestamp_10_test() {
  assert timestamp.parse_http_date("Mon, 29 jun 2026 01:49:08 GMT")
    == Error(Nil)
}

pub fn parse_http_date_returns_error_for_bad_timestamp_11_test() {
  assert timestamp.parse_http_date("Mon, 29-Jun-2026 01:49:08 GMT")
    == Error(Nil)
}

pub fn parse_http_date_returns_error_for_bad_timestamp_12_test() {
  assert timestamp.parse_http_date("Monday, 06 Nov 1994 08:49:37 GMT")
    == Error(Nil)
}

pub fn parse_http_date_returns_error_for_bad_timestamp_13_test() {
  assert timestamp.parse_http_date("Mon, 06-Nov-94 08:49:37 GMT") == Error(Nil)
}

pub fn parse_http_date_returns_error_for_bad_timestamp_14_test() {
  assert timestamp.parse_http_date("Monday Nov  6 08:49:37 1994") == Error(Nil)
}

pub fn timestamp_http_date_string_timestamp_roundtrip_property_test() {
  use original_timestamp <- qcheck.given(generators.timestamp())
  let #(seconds, _) =
    timestamp.to_unix_seconds_and_nanoseconds(original_timestamp)
  let original_timestamp = timestamp.from_unix_seconds(seconds)

  let assert Ok(parsed_timestamp) =
    original_timestamp
    |> timestamp.to_http_date
    |> timestamp.parse_http_date

  assert original_timestamp == parsed_timestamp
}
